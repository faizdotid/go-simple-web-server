...
func (p *postModel) Create(user_id int, data Post) (Post, error) {
	...
	if user_id == 0 {
		stmtstring = "INSERT INTO posts(title, content) VALUES(?, ?)"
	} else {
		stmtstring = "INSERT INTO posts(title, content, user_id) VALUES(?, ?, ?)"
	}
	stmt, err := p.conn.Prepare(stmtstring)
	if err != nil {
		return Post{}, err
	}
	if user_id == 0 {
		_, err = stmt.Exec(data.Title, data.Content)
	} else {
